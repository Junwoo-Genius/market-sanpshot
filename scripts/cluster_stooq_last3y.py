...
def fetch_stooq_daily(ticker: str):
    ...
    meta = {
        ...
        "date_range": [df["Date"].min().strftime("%Y-%m-%d"), df["Date"].max().strftime("%Y-%m-%d")] if len(df) else None,
    }
    return df, meta
...
def main():
    run_meta = {
        ...
        "source": "stooq_csv",
        "source_pattern": "https://stooq.com/q/d/l/?s={ticker}.us&i=d",
        "universe": TICKERS,
        "window": "last_3y",
        "k": 5,
        "features": [
    ...
    for t in TICKERS:
        try:
            ...
            meta["last3y_date_range"] = [df3["Date"].min().strftime("%Y-%m-%d"), df3["Date"].max().strftime("%Y-%m-%d")] if len(df3) else None
    ...
    X = feat_df[[c for c in run_meta["features"] if not c.startswith("_")]].replace([np.inf, -np.inf], np.nan).dropna()
    used = list(X.index)
    ...
    sep = {}
    for col in [c for c in run_meta["features"] if not c.startswith("_")]:
    ...
    readme.append("- source_fingerprints.json (sha256/row_count/date_range per ticker)")
    ...
    readme.append("")
    readme.append("## Note")
    readme.append("- This is a *proposal* generator. Do not auto-update your rulebook without explicit approval.")
